...
CREATE_MARKET_OUTPUT=`  target/debug/solend-program  --program $PROGRAM_ID create-market \
  --fee-payer    $OWNER \
...
USDC_RESERVE_OUTPUT=`  target/debug/solend-program  --program $PROGRAM_ID add-reserve \
  --fee-payer         $OWNER \
  --market-owner      $OWNER \
  --source-owner      $OWNER \
  --market            $MARKET_ADDR \
  ...
  --deposit-limit 1000000 --protocol-take-rate 1 --protocol-liquidation-fee 1  \
  --verbose`;
...
ETH_RESERVE_OUTPUT=`  target/debug/solend-program  --program $PROGRAM_ID add-reserve \
  --fee-payer         $OWNER \
  --market-owner      $OWNER \
  --source-owner      $OWNER \
  --market            $MARKET_ADDR \
  ...
  --max-borrow-rate 100 \
  --host-fee-percentage 50 \
  --deposit-limit 500 --protocol-take-rate 1 --protocol-liquidation-fee 1 \
  --verbose`;
...
BTC_RESERVE_OUTPUT=`  target/debug/solend-program  --program $PROGRAM_ID add-reserve \
  --fee-payer         $OWNER \
  --market-owner      $OWNER \
  --source-owner      $OWNER \
  --market            $MARKET_ADDR \
  ...
  --max-borrow-rate 100 \
  --host-fee-percentage 50 \
  --deposit-limit 30 --protocol-take-rate 1 --protocol-liquidation-fee 1 \
  --verbose`;
